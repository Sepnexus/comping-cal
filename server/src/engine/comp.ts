...
export async function runComp(input: RunCompInput): Promise<CompOutcome> {
  ...
  // Call Bricked first — get the valuation before any charge (R4 / §6.2 step 2).
  // Default the comp lookback to the admin setting; an explicit override wins.
  ...
  if (!result.ok) {
    // Failed Bricked call never charges the wallet (R4). Append-only log, no charge.
    usage.insert({
      location_id: loc.id,
      snapshot_id: null,
      ...
      address: input.address,
      idempotency_key: null, // allow free retry until it succeeds once (§6.3)
    });
    return { ok: false, status: result.status, charged: false, chargeStatus: 'not_attempted', fallback: fallbackFor(result.status) };
  }

  // HTTP 200 → charge the comp via the agency billing endpoint (§6.2 step 3).
  ...
  if (!charge.ok) {
    // Charge declined: do NOT deliver a fresh paid result; billing-issue state. We
    // absorb at most the single in-flight Bricked call (§6.2 step 5).
    usage.insert({
      location_id: loc.id,
      snapshot_id: null,
      type: isRefresh ? 'refresh' : 'comp',
      ...
      address: input.address,
      // No charge occurred, so the attempt does not claim the idempotency key —
      // the user may retry once billing is resolved (§4.7 resolve → active).
      idempotency_key: null,
    });
    return { ok: false, status: 200, charged: false, chargeStatus: 'charge_failed', billingIssue: true, fallback: { kind: 'billing_issue', message: 'There’s a billing issue on this account — your last charge was declined.', reason: charge.reason } };
  }

  // Charge succeeded → store snapshot + log usage (§6.2 step 4). Snapshot is the
  // universal-fallback anchor: once stored, the delivered valuation is never lost.
